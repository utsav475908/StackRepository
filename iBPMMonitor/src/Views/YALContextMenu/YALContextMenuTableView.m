...
- (void)show:(BOOL)show visibleCellsAnimated:(BOOL)animated {
    ...
    UITableViewCell<YALContextMenuCell> *visibleCell
        = (UITableViewCell<YALContextMenuCell> *)[self cellForRowAtIndexPath:animatingIndexPath];
    if (visibleCell) {
        ...
        [self show:show cell:visibleCell
          animated:animated
         direction:direction
         clockwise:NO
...
- (void)dismissSelf {
    ...
    BOOL clockwise = self.menuItemsSide == Right ? NO : YES;
        [self show:NO cell:self.selectedCell
          animated:YES
         direction:direction
         clockwise:clockwise
        completion:^(BOOL completed) {
